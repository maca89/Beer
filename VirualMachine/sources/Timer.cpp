...
void TimerClassInitializer::initClass(Thread* thread, ClassLoader* loader, Class* klass)
{
	...
	loader->addVirtualMethod(thread, klass, BEER_WIDEN("Timer"), BEER_WIDEN("Timer::Timer()"), &Timer::init, 1, 0);
	loader->addVirtualMethod(thread, klass, BEER_WIDEN("start"), BEER_WIDEN("Timer::start()"), &Timer::start, 0, 0);
	loader->addVirtualMethod(thread, klass, BEER_WIDEN("stop"), BEER_WIDEN("Timer::stop()"), &Timer::stop, 1, 0);

	loader->addOverrideMethod(thread, klass, BEER_WIDEN("createInstance"), BEER_WIDEN("$Class::createInstance()"), &Timer::createInstance, 1, 0);
}