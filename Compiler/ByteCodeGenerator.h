...
namespace BeerlangCompiler
{
	namespace CodeGeneration
	{
		...
		protected:
			...
			SemanticAnalysis::ClassInfo * mClass;
			SemanticAnalysis::MethodInfo * mMethod;
			SemanticAnalysis::BlockInfo * mBlock;
		...
		public:
			...
			ByteCodeGenerator(std::ostream & out, SemanticAnalysis::MethodInfo * method, SemanticAnalysis::BlockInfo * block, bool reverseBytes = false);
			ByteCodeGenerator(std::ostream & out, SemanticAnalysis::MethodInfo * method, SemanticAnalysis::BlockInfo * block, count instructionOffset, bool reverseBytes = false);


			void generate(SemanticAnalysis::MethodInfo * method, SemanticAnalysis::BlockInfo * block);
		...
		protected:
			...
			INLINE void generateINVOKE(uint32 id)
			{
			...
			INLINE void generateSPECIAL_INVOKE(uint32 id)
			{
			...
			INLINE void generateRETURN(uint16 count)
			{
				mBw.write((instruction)RETURN);
				mBw.write(count);
				mInstructionCount++;
				mInstructionLength += sizeof(instruction) + sizeof(count);
				//changeStackSize(-static_cast<int16>(count));
			}
		};
	}
}